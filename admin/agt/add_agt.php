...
if (strlen($tambah) == 1) {
    ...
    $format = "A" . "0" . $tambah;
} else { // tidak perlu syarat tambahan di else
    $format = "A" . $tambah;
}
?>
...
<?php
    if (isset($_POST['Simpan'])) {
        ...
        // Validasi form
        if (empty($_POST['nama']) || empty($_POST['jekel']) || empty($_POST['alamat']) || empty($_POST['no_hp'])) {
            // Jika ada field yang kosong
            echo "<script>
            Swal.fire({title: 'Tambah Data Gagal',text: 'Semua field harus diisi.',icon: 'error',confirmButtonText: 'OK'
            }).then((result) => {
                if (result.value) {
